...
<body>
    ...
    <?php if (isset($_POST['btn-signin'])): ?>
        <?php
            ...
            $usernInput = $_POST['inputUsername'];
            $userpInput = $_POST['inputPassword'];
            
            $isValidUser = isset($ValidCredentials[$selectedType][$usernInput]) && $ValidCredentials[$selectedType][$usernInput] === $userpInput;
        ?>

        <?php if ($isValidUser): ?>
            <div class="alert alert-success alert-dismissible fade show" style="max-width: 350px; margin: auto;">   
                Welcome to the system, <?php echo htmlspecialchars($usernInput); ?>!
            </div>
        <?php else: ?>
